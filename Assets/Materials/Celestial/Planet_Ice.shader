		...
		SubShader
		{
			...
			CGINCLUDE
				...
				float groundHeight(const float3 p) {
					...
					float height = (map.r + map.g + lumps) * -_Scale;
					if (height < _SeaLevel) return _SeaLevel;
					return height;
				}
				...
				half3 atmoColor(float horizon, float sunDot) {
					...
					const half3 skyColor = half3(0.1, 0.1, 0.1);
				...
				half4 addClouds(float3 ro, float3 rd, float dist, float2 air) {
					...
					const half3 color = half3(0.5, 0.5, 0.5);// * nightSide;
